// D:\flutter sdk\aplikasi_diagnosa_gizi\lib\src\features\account_page\logic\backup_restore_service.dart
    ...
    try {
      ...
      for (var p in selectedDewasa) {
        rows.add([
          ...
          p.detailAlergi ?? '-',
          p.polaMakan ?? '-',
          p.sukaManis == true ? 'Ya' : 'Tidak',
          p.sukaAsin == true ? 'Ya' : 'Tidak',
          p.makanBerlemak == true ? 'Ya' : 'Tidak',
          p.jarangOlahraga == true ? 'Ya' : 'Tidak',
          p.imt.toStringAsFixed(2),
          p.skorIMT,
          p.skorKehilanganBB,
          p.skorEfekPenyakit,
          p.totalSkor,
          p.monevStatusGizi ?? '-',
          formatLab(p.labResults),
          p.klinikTD ?? '-',
          p.klinikNadi ?? '-',
          p.klinikSuhu ?? '-',
          p.klinikRR ?? '-',
          ...
          p.klinikKU ?? '-',
          p.klinikKES ?? '-',
          p.riwayatPenyakitSekarang ?? '-',
          p.riwayatPenyakitDahulu ?? '-',
          p.diagnosaGizi ?? '-',
          p.intervensiDiet ?? '-',
          p.intervensiBentukMakanan ?? '-',
          p.intervensiVia ?? '-',
          p.intervensiTujuan ?? '-',
          p.monevIndikator ?? '-',
          p.monevAsupan ?? '-',
          p.monevHasilLab ?? '-',
          formatDate(p.tanggalPemeriksaan),
        ]);
      }
      ...
      rows.add([
        ...
        'BBI (kg)',
        'Skor Kehilangan BB',
        'Skor Kehilangan Nafsu Makan',
        'Skor Anak Sakit Berat',
        'Nama Nutrisionis',
        'Alergi Makanan',
        'Pola Makan',
        'Z-Score BB/U',
