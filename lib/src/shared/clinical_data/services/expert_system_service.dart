...
class DiseaseExpertService {
  ...
  ExpertSystemResult generateCarePlan(ExpertSystemInput input) {
    ...
    final bool isHD = isKidney &&
        (input.isHemodialysis ||
         (_containsKeyword(input.medicalDiagnosis, ['hemodialisis', 'hd', 'cuci darah'])));
    ...
    // Anemia
    final bool isAnemia = input.hgb != null &&
    ...
    if (input.imt >= 30) {
      _addDiagnosis(diagnoses, 'NC-3.3',
      ...
      _addDiagnosis(diagnoses, 'NC-3.1',
    ...
    if (isKidney) {
      ...
      _addDiagnosis(diagnoses, 'NC-2.2',
          customDef: 'Gangguan fungsi ginjal (CKD${isHD ? " + HD" : " Pre-HD"}). $renalDetail');
    }
    ...
    if (isDiabetes && (input.sukaManis ||
        _containsKeyword(input.medicalDiagnosis, ['manis']))) {
      _addDiagnosis(diagnoses, 'NI-5.8.3',
          customDef: 'Konsumsi gula murni/makanan manis berlebih (kontraindikasi DM)');
    }

    // Asupan karbohidrat tidak konsisten (DM)
    if (isDiabetes) {
      _addDiagnosis(diagnoses, 'NI-5.8.4',
          customDef: 'Pola makan tidak teratur — perlu prinsip 3J (Jadwal, Jumlah, Jenis)');
    }

    // Asupan serat kurang (DM)
    if (isDiabetes) {
      _addDiagnosis(diagnoses, 'NI-5.8.5',
    ...
    if ((isDiabetes || isDislipidemia) && input.makanBerlemak) {
      _addDiagnosis(diagnoses, 'NI-5.6.3',
          customDef: 'Asupan lemak jenuh > 10% kebutuhan energi / kolesterol > 300 mg/hari');
    }

    // Kelebihan natrium (Hipertensi / CKD)
    if ((isHypertension || isKidney) &&
        (input.sukaAsin ||
         _containsKeyword(input.dietaryHistory, ['asin', 'awetan', 'ikan asin', 'telor asin']))) {
    ...
    if (isKidney && !isHD) {
      _addDiagnosis(diagnoses, 'NI-5.4',
          customDef:
    ...
    if (isHD) {
      _addDiagnosis(diagnoses, 'NI-5.1',
          customDef:
    ...
    if (isKidney) {
      _addDiagnosis(diagnoses, 'NI-3.2',
          customDef:
    ...
    // Hipoalbuminemia
    if (isHipoalbumin) {
      _addDiagnosis(diagnoses, 'NI-5.3',
          customDef:
    ...
    if (input.jarangOlahraga ||
        _containsKeyword(input.dietaryHistory, ['jarang olahraga', 'tidak olahraga'])) {
    ...
    if (isDiabetes || isKidney) {
      _addDiagnosis(diagnoses, 'NB-1.7',
          customDef: 'Pemilihan jenis/jumlah/jadwal makanan tidak sesuai kondisi penyakit');
    }
    ...
    if (isHD) {
      ...
      _addIntervention(interventions, 'ND-3.1.1',
          note:
    ...
    // ── HIPERKALEMIA (CKD) ───────────────────────────────────────────────────
    if (isHiperkalemia) {
      _addIntervention(interventions, 'ND-1.3',
          note:
    ...
    // ── ANEMIA ────────────────────────────────────────────────────────────────
    if (isAnemia) {
      _addIntervention(interventions, 'ND-3.2.4',
          note:
    ...
    _addIntervention(interventions, 'NE-2.2',
        note: 'Edukasi mendalam: cara membaca label pangan, estimasi porsi, pengganti bahan makanan.');

    // ── Konseling Gizi ────────────────────────────────────────────────────────
    _addIntervention(interventions, 'NC-1.4',
        note: 'Konseling motivasi: tahap kesiapan perubahan perilaku diet.');
    _addIntervention(interventions, 'NC-2.2',
    ...
    // Asupan Makronutrien
    _addMonitoring(monitoring, 'FI-5.2',
        note: isDiabetes
    ...
    _addMonitoring(monitoring, 'FI-5.3',
        note: isDiabetes
            ? 'Asupan KH (60–70% energi); pantau konsumsi gula sederhana'
            : 'Asupan Karbohidrat');

    _addMonitoring(monitoring, 'FI-5.1',
        note: 'Asupan Lemak (jenuh < 10% energi; kolesterol ≤ 300 mg/hari)');

    if (isDiabetes) {
      _addMonitoring(monitoring, 'FI-5.4',
          note: 'Asupan Serat (target ≥ 25 g/hari — DM)');
    }

    // Glukosa Darah (DM)
    if (isDiabetes) {
      _addMonitoring(monitoring, 'S-2.5',
          note: 'GDS, GDP, HbA1c — target GDS < 200 mg/dL, GDP < 126 mg/dL, HbA1c < 6.5%');
    }

    // Profil Ginjal & Elektrolit (CKD)
    if (isKidney) {
      _addMonitoring(monitoring, 'S-2.2',
          note: 'Ureum, Kreatinin, GFR, Natrium, Kalium, Fosfor, Kalsium');
      _addMonitoring(monitoring, 'S-3.1',
          note: 'Berat Badan Kering, Edema, Balance Cairan (timbang setiap kunjungan)');
    }

    // Profil Lipid (Dislipidemia / DM / CKD)
    if (isDislipidemia || isDiabetes || isKidney) {
      _addMonitoring(monitoring, 'S-2.7',
          note: 'Kolesterol Total, LDL, HDL, Trigliserida');
    }

    // Tekanan Darah (Hipertensi)
    if (isHypertension) {
      _addMonitoring(monitoring, 'S-1.1',
  ...
  bool _isDiabetes(ExpertSystemInput i) =>
      _containsKeyword(i.medicalDiagnosis, ['diabetes', 'dm', 'dm tipe', 'kencing manis']) ||
  ...
  bool _isKidney(ExpertSystemInput i) =>
      ...
      _containsKeyword(i.medicalDiagnosis, [
        'ckd', 'ginjal kronik', 'chronic kidney', 'penyakit ginjal',
        'hemodialisis', 'cuci darah', 'uremia',
      ]) ||
      (i.ureum     != null && i.ureum!    > 50)  ||
      (i.kreatinin != null && i.kreatinin! > 1.5);

  bool _isHypertension(ExpertSystemInput i) {
    if (_containsKeyword(i.medicalDiagnosis, ['hipertensi', 'hypertensi', 'tekanan darah tinggi'])) return true;
    if (i.bloodPressure.toLowerCase().contains('hipertensi') ||i.bloodPressure.toLowerCase().contains('tinggi')) return true;
  ...
  bool _isMale(ExpertSystemInput i) =>
      !_containsKeyword(i.medicalDiagnosis, ['perempuan', 'wanita', 'ny.', 'ny ', 'nn.']);
  ...
  void _addDiagnosis(
    ...
    try {
      final found = DiagnosisTerminology.allDiagnoses
  ...
  void _addIntervention(
    ...
    try {
      final found = IntervensiData.allInterventions
          .firstWhere((item) => item.code == code);
      list.add(TerminologyItem(
        domain:    found.domain,
        classCode: found.classCode,
  ...
  void _addMonitoring(
      List<TerminologyItem> list, String code, {String? note}) {
    try {
      final found = MonitoringData.allMonitoringItems
          .firstWhere((item) => item.code == code);
      list.add(TerminologyItem(
        domain:    found.domain,
        classCode: found.classCode,
  ...
  ExpertSystemResult buildAndRun({
    ...
    required Map<String, String> labValues, // key = jenis lab, value = teks angka
    // ── Klinis ───────────────────────────────────────────────────────────────
  ...
  static String buildSigns(
    ...
    double? gds, double? gdp, double? hba1c,
    double? ureum, double? kreatinin, double? kolesterol,
    double? ldl, double? hdl, double? trigliserida,
    double? kalium, double? natrium, double? hgb,
    ...
    switch (diag.code) {
      case 'NC-2.2':
        final parts = <String>[];
        if (gds          != null) parts.add('GDS: ${gds.round()} mg/dL');
        if (gdp          != null) parts.add('GDP: ${gdp.round()} mg/dL');
        if (hba1c        != null) parts.add('HbA1c: $hba1c%');
        if (ureum        != null) parts.add('Ureum: ${ureum.round()} mg/dL');
        if (kreatinin    != null) parts.add('Kreatinin: $kreatinin mg/dL');
        if (kolesterol   != null) parts.add('Kol.Total: ${kolesterol.round()} mg/dL');
        if (ldl          != null) parts.add('LDL: ${ldl.round()} mg/dL');
        if (hdl          != null) parts.add('HDL: ${hdl.round()} mg/dL');
        if (trigliserida != null) parts.add('TG: ${trigliserida.round()} mg/dL');
        if (kalium       != null) parts.add('Kalium: $kalium mEq/L');
      ...
      case 'NI-5.10.2':
        final parts = <String>[];
        if (sukaAsin)                         parts.add('riwayat suka asin');
        if (kalium   != null)                 parts.add('Kalium: $kalium mEq/L');
        if (natrium  != null)                 parts.add('Natrium: $natrium mEq/L');
        if (sistolik != null && diastolik != null) parts.add('TD: $sistolik/$diastolik mmHg');
        return parts.join('; ');
      case 'NI-5.10.1':
