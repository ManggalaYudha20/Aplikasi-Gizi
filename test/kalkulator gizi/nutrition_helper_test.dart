...
void main() {
  group('Unit Test: Akurasi Z-Score Status Gizi Balita (0-60 Bulan)', () {
    ...
    test('Skenario Bayi Laki-laki Baru Lahir (0 Bulan) - Tumbuh Kembang Median (Normal)', () {
      ...
      final results = NutritionCalculatorService.calculateAll(
        birthDate: DateTime(2023, 1, 1),
        checkDate: DateTime(2023, 1, 1), 
        weightKg: 3.3, // Parameter diubah menjadi weightKg
        heightCm: 49.9, // Parameter diubah menjadi heightCm
        gender: 'Laki-laki',
      );
      ...
      final bbU = results.weightForAge.zScore as double;
      expect(bbU, closeTo(0.0, 0.05));
      expect(results.weightForAge.category, 'Normal');

      // Verifikasi Z-Score TB/U
      final tbU = results.heightForAge.zScore as double;
      expect(tbU, closeTo(0.0, 0.05));
      expect(results.heightForAge.category, 'Normal');

      // Verifikasi Z-Score BB/TB (Tinggi terdekat di tabel adalah 50.0 cm yang median BB-nya 3.3 kg)
      final bbTb = results.weightForHeight.zScore as double;
      expect(bbTb, closeTo(0.0, 0.05));
      expect(results.weightForHeight.category, 'Gizi baik');
    });
    ...
    test('Skenario Balita Perempuan 12 Bulan - Risiko Gizi Kurang', () {
      ...
      final results = NutritionCalculatorService.calculateAll(
        ...
        weightKg: 7.0, // Parameter diubah menjadi weightKg
        heightCm: 70.0, // Parameter diubah menjadi heightCm
        gender: 'Perempuan',
      );
      ...
      expect(results.weightForAge.category, contains('Kurang'));
    });
  });
}